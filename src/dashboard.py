...
#     i18n.py             -> Arabic/English sentences + page direction
#     extract_cv_data.py  -> reads + scores the CV files
#     ui_components.py    -> filters, results table, contact, preview
#     analytics_ui.py     -> charts + per-candidate analytical summary
#     settings_ui.py      -> the ⚙️ AI-providers settings tab
#     ai_insights.py      -> AI analysis of the selected candidate
# =====================================================================
...
# ---------------------------------------------------------------------
# 2. CACHED LOADER (kept at module level so the cache is shared)
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# 3. THE ANALYSIS TAB (the original dashboard flow)
# ---------------------------------------------------------------------
def render_analysis_tab() -> None:
    # --- choose where the CVs come from: project folder OR upload ---
    ...
    if source == "upload":
        ...
        # On a server, visitors may NOT type arbitrary paths — that
        # would let them browse the server's disk. They can only pick
        # sub-folders of the approved data folder (DATA_ROOT). On your
        # own PC set CV_ANALYZER_ALLOW_ANY_PATH=1 to unlock free input.
        if ALLOW_ANY_PATH:
    ...
    # --- matching requirements: job description + structured criteria ---
    ...
    # --- load & score the CVs (cached so repeat visits are instant) ---
    ...
    # --- filters -> results -> analytics -> candidate analysis -> AI ---
    ...
    render_ai_insights(selected_row, matching_text)
    render_cv_preview(selected_row)


# ---------------------------------------------------------------------
# 4. TABS: analysis | AI settings
# ---------------------------------------------------------------------
tab_analysis, tab_settings = st.tabs([t("tab_analysis"), t("tab_settings")])
with tab_analysis:
    render_analysis_tab()
with tab_settings:
